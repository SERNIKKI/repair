...
public class MAreaInfoQueryByBuildingServlet extends HttpServlet {
    @Override
...
//        super.doPost(req, resp);
        ...
        building = new String(building.getBytes("ISO-8859-1"),"UTF-8");

        JsonData<List<AreaInfo>> jsonData = new JsonData<>();
        if(token!=null){
            if(token.equalsIgnoreCase(req.getSession().getId())){
                RepairDao repairDao = new RepairDao();
                if(building!=null&&!building.isEmpty()){
                    try {
                        List<AreaInfo> list = repairDao.selectFloor(building);
                        //发送消息
                        jsonData.setCode(0);
                        jsonData.setMsg("成功");
